...
type, extends(base_object), abstract :: interpolator_object
  ...
  contains
    ! public methods
    generic :: interpolate => interpolate_stencil_rank_1_debug,    interpolate_stencil_rank_2_debug, &
                              interpolate_stencil_rank_1_standard, interpolate_stencil_rank_2_standard
    ! public deferred methods
    procedure(interpolate_stencil_rank_1_debug_interface),    pass(self), deferred :: interpolate_stencil_rank_1_debug
    procedure(interpolate_stencil_rank_2_debug_interface),    pass(self), deferred :: interpolate_stencil_rank_2_debug
    procedure(interpolate_stencil_rank_1_standard_interface), pass(self), deferred :: interpolate_stencil_rank_1_standard
    procedure(interpolate_stencil_rank_2_standard_interface), pass(self), deferred :: interpolate_stencil_rank_2_standard
endtype interpolator_object

abstract interface
  !< Abstract interfaces of [[interpolator_object]].
  pure subroutine interpolate_stencil_rank_1_debug_interface(self, stencil, interpolation, si, weights)
  ...
  endsubroutine interpolate_stencil_rank_1_debug_interface

  pure subroutine interpolate_stencil_rank_2_debug_interface(self, stencil, interpolation, si, weights)
  !< Interpolate values (providing also debug values).
  import :: interpolator_object, RPP
  class(interpolator_object), intent(inout) :: self                     !< Interpolator.
  ...
  endsubroutine interpolate_stencil_rank_2_debug_interface

  pure subroutine interpolate_stencil_rank_1_standard_interface(self, stencil, interpolation)
  !< Interpolate values (without providing debug values).
  import :: interpolator_object, RPP
  class(interpolator_object), intent(inout) :: self                 !< Interpolator.
  real(RPP),                  intent(in)    :: stencil(1 - self%S:) !< Stencil of the interpolation [1-S:-1+S].
  real(RPP),                  intent(out)   :: interpolation        !< Result of the interpolation.
  endsubroutine interpolate_stencil_rank_1_standard_interface

  pure subroutine interpolate_stencil_rank_2_standard_interface(self, stencil, interpolation)
  !< Interpolate values (without providing debug values).
  import :: interpolator_object, RPP
  class(interpolator_object), intent(inout) :: self                     !< Interpolator.
  real(RPP),                  intent(in)    :: stencil(1:, 1 - self%S:) !< Stencil of the interpolation [1:2, 1-S:-1+S].
  real(RPP),                  intent(out)   :: interpolation(1:)        !< Result of the interpolation, [1:2].
  endsubroutine interpolate_stencil_rank_2_standard_interface
endinterface
