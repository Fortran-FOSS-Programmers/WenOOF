...
contains
  ...
  real(RPP)                        :: alpha_base(0:self%S-1) !< Alpha base coefficients.
  real(RPP)                        :: alpha_base_sum         !< Sum of alpha base coefficients.
  ...
  do s1=0, self%S - 1 ! stencil loops
    ...
    values(s1) =                                                                                                       &
      (kappa_base * (kappa(s1) + kappa(s1) * kappa(s1) - 3._RPP * kappa(s1) * kappa_base + kappa_base * kappa_base)) / &
      (kappa(s1) * kappa(s1) + kappa_base * (1._RPP - 2._RPP * kappa(s1)))
  enddo
