...
private
public :: alpha_int_z
public :: alpha_int_z_constructor
...
contains
  ...
  class(alpha_int_z),  intent(in)  :: self       !< Alpha.
  ...
  do s1=0, self%S - 1 ! stencil loops
    values = kappa(s1) * ((1._RPP + (tau(S=self%S, beta=beta) / (self%eps + beta(s1)))) ** (weno_exp(self%S)))
  enddo
  endsubroutine compute_int

  pure subroutine compute_rec(self, beta, kappa, values)
  !< Compute alpha (reconstruct).
  class(alpha_int_z), intent(in)  :: self          !< Alpha coefficient.
