...
type, extends(interpolator_constructor) :: reconstructor_js_constructor
...
type, extends(interpolator) :: reconstructor_js
  ...
  contains
    ...
    procedure, nopass     :: description          !< Return reconstructor string-description.
...
contains
  ! public non TBP
  subroutine create_reconstructor_js_constructor(is, alpha, weights, polynom, S, constructor, eps)
  !< Return an instance of [reconstructor_js_constructor].
  ...
  class(interpolator_constructor), allocatable, intent(out)          :: constructor !< reconstructor constructor.
  ...
  class is(reconstructor_js_constructor)
    constructor%S = S
  ...
  string = 'Jiang-Shu WENO upwind-biased reconstructor'//nl
